...
{
	printf("Person table(Name:WStr(10), ID;Integer, Address:WStr(15), Key:Str(10), Height:Float) Max=16383 can be generated by CreateTable.");
	...
	printf("16383 records can be added to Person table with 16383 repeat of InsertRecord.");
	...
	{
		printf("-1 is returned if sorting of (AzSortRecord(L\"Person\", L\"ID\")) is performed without locking.");
		...
		printf("-1 is returned if sorting of (ZaSortRecord(L\"Person\", L\"ID\")) is performed without locking.");
		...
		printf("-1 is returned if sorting of (AzSortRecord(L\"Person\", L\"ID\")) is performed after shared lock.");
		...
		printf("-1 is returned if sorting of (ZaSortRecord(L\"Person\", L\"ID\")) is performed after shared lock.");
		LockTable(L"Person", LOCK_SHARE);
		if (ZaSortRecord(L"Person", L"ID") != -1) {
			UnlockTable(L"Person");
			printf("...[NG]\r\n");
		...
		printf("0 is returned if sorting of (AzSortRecord(L\"Person\", L\"ID\")) is performed after exclusive lock.");
		...
		printf("0 is returned if sorting of (ZaSortRecord(\"Person\", L\"ID\")) is performed after exclusive lock.");
		LockTable(L"Person", LOCK_EXCLUSIVE);
	...
	{
		printf("Connected 16383 records can be acquired by GetRecord(table name specification) from 16383 records which are added.");
		int Count = 0;
	...
	{
		printf("Table can be deleted using DeleteTable(\"Person\").");
