...
{
	StkObject* Elem1 = new StkObject(_T("å¬êlÇÃç‡éY"));

	if (Mode == 1) {
		StkObject* Elem1_3 = new StkObject(_T("åªã‡"));
		Elem1_3->AppendChildElement(new StkObject(_T("Cã‚çs"), 7000000));
		Elem1_3->AppendChildElement(new StkObject(_T("Aã‚çs"), 5000000));
		Elem1_3->AppendChildElement(new StkObject(_T("Bã‚çs"), 10000000));
		Elem1->AppendChildElement(Elem1_3);

		StkObject* Elem1_2 = new StkObject(_T("éûåv"));
		Elem1_2->AppendChildElement(new StkObject(_T("ÉçÉåÉbÉNÉX"), _T("ã‡")));
		Elem1_2->AppendChildElement(new StkObject(_T("ÉIÉÅÉK"), _T("çï")));
		Elem1_2->AppendChildElement(new StkObject(_T("ÉOÉâÉìÉhÉZÉCÉRÅ["), _T("îí")));
		Elem1->AppendChildElement(Elem1_2);

		StkObject* Elem1_1 = new StkObject(_T("èÊópé‘"));
		StkObject* Elem1_1_2 = new StkObject(_T("ÉuÉãÅ[ÉoÅ[Éh"));
		Elem1_1_2->AppendAttribute(new StkObject(_T("ÉÅÅ[ÉJÅ["), _T("ì˙éY")));
		Elem1_1_2->AppendAttribute(new StkObject(_T("êF"), _T("îí")));
		Elem1_1->AppendChildElement(Elem1_1_2);
		StkObject* Elem1_1_1 = new StkObject(_T("ÉJÉçÅ[Éâ"));
		Elem1_1_1->AppendAttribute(new StkObject(_T("ÉÅÅ[ÉJÅ["), _T("ÉgÉàÉ^")));
		Elem1_1_1->AppendAttribute(new StkObject(_T("êF"), _T("ê‘")));
	...
	StkObject* Elem1_4 = new StkObject(_T("èÿåî"));
	StkObject* Elem1_4_1 = new StkObject(_T("äî"));
	Elem1_4_1->AppendChildElement(new StkObject(_T("Aé–"), 2000000));
	Elem1_4_1->AppendChildElement(new StkObject(_T("Bé–"), 500000));
	Elem1_4->AppendChildElement(Elem1_4_1);
	StkObject* Elem1_4_2 = new StkObject(_T("çëç¬"));
	if (Mode == 1) {
		Elem1_4_2->AppendChildElement(new StkObject(_T("ç¬å†"), 1000000));
		Elem1_4_2->AppendChildElement(new StkObject(_T("ç¬å†"), 2000000));
		Elem1_4_2->AppendChildElement(new StkObject(_T("ç¬å†"), 1500000));
	} else {
		Elem1_4_2->AppendChildElement(new StkObject(_T("ç¬å†"), 2000000));
		Elem1_4_2->AppendChildElement(new StkObject(_T("ç¬å†"), 1500000));
		if (Mode == 2) {
			Elem1_4_2->AppendChildElement(new StkObject(_T("ç¬å†"), 1000001));
		} else {
			Elem1_4_2->AppendChildElement(new StkObject(_T("ç¬å†"), 1000000));
		}
	}
	Elem1_4->AppendChildElement(Elem1_4_2);
	StkObject* Elem1_4_3 = new StkObject(_T("ã‡åî"));
...
void GeneralTestCase2(StkObject* Elem1, TCHAR* Name)
{
	...
	if (lstrcmp(Elem1->GetName(), _T("å¬êlÇÃç‡éY")) != 0 ||
		lstrcmp(Elem1->GetFirstChildElement()->GetName(), _T("èÊópé‘")) != 0 ||
		lstrcmp(Elem1->GetFirstChildElement()->GetNext()->GetName(), _T("éûåv")) != 0 ||
		lstrcmp(Elem1->GetFirstChildElement()->GetNext()->GetNext()->GetName(), _T("åªã‡")) != 0 ||
		lstrcmp(Elem1->GetFirstChildElement()->GetNext()->GetNext()->GetNext()->GetName(), _T("èÿåî")) != 0) {
		printf("NG\r\n");
		exit(0);
	}
	printf("OK\r\n");
	...
	if (lstrcmp(Elem1->GetFirstChildElement()->GetFirstChildElement()->GetFirstAttribute()->GetStringValue(), _T("ê‘")) != 0 ||
		lstrcmp(Elem1->GetFirstChildElement()->GetFirstChildElement()->GetFirstAttribute()->GetNext()->GetStringValue(), _T("ÉgÉàÉ^")) != 0 ||
		lstrcmp(Elem1->GetFirstChildElement()->GetFirstChildElement()->GetNext()->GetFirstAttribute()->GetStringValue(), _T("îí")) != 0 ||
		lstrcmp(Elem1->GetFirstChildElement()->GetFirstChildElement()->GetNext()->GetFirstAttribute()->GetNext()->GetStringValue(), _T("ì˙éY")) != 0) {
		printf("NG\r\n");
		exit(0);
	}
	printf("OK\r\n");
...
void GeneralTestCase4()
{
	...
	}
	{
		...
		Abort(Ret != NULL && lstrcmp(Ret->GetName(), _T("èÿåî")) == 0 && Ret->GetFirstChildElement()->GetFirstChildElement()->GetIntValue() == 2000000);
		delete Origin;
		delete Elem1_4;
	}
	{
...
void JsonEncodingTest2()
{
	...
	if (lstrcmp(JsonTxt2, _T("\"å¬êlÇÃç‡éY\" : {\r\n  \"èÊópé‘\" : {\r\n    \"")) != 0) {
		printf("NG\r\n");
		exit(0);
	}
	lstrcpy(JsonTxt2, _T(""));
...
void PerformanceTest()
{
	...
	for (int Loop = 2; Loop < 10; Loop++) {
		lstrcpy(JsonTxt, _T(""));
		StkObject* ObjWid = MakeTestData3(_T("Ç†Ç¢Ç§Ç¶Ç®Ç©Ç´Ç≠ÇØÇ±Ç≥ÇµÇ∑ÇπÇªÇΩÇøÇ¬ÇƒÇ∆Ç»Ç…Ç ÇÀÇÃÇÕÇ–Ç”Ç÷ÇŸ"), 2, Loop);
