...
{
	StkObject* Elem1 = new StkObject(L"å¬êlÇÃç‡éY");

	if (Mode == 1) {
		StkObject* Elem1_3 = new StkObject(L"åªã‡");
		Elem1_3->AppendChildElement(new StkObject(L"Cã‚çs", 7000000));
		Elem1_3->AppendChildElement(new StkObject(L"Aã‚çs", 5000000));
		Elem1_3->AppendChildElement(new StkObject(L"Bã‚çs", 10000000));
		Elem1->AppendChildElement(Elem1_3);

		StkObject* Elem1_2 = new StkObject(L"éûåv");
		Elem1_2->AppendChildElement(new StkObject(L"ÉçÉåÉbÉNÉX", L"ã‡"));
		Elem1_2->AppendChildElement(new StkObject(L"ÉIÉÅÉK", L"çï"));
		Elem1_2->AppendChildElement(new StkObject(L"ÉOÉâÉìÉhÉZÉCÉRÅ[", L"îí"));
		Elem1->AppendChildElement(Elem1_2);

		StkObject* Elem1_1 = new StkObject(L"èÊópé‘");
		StkObject* Elem1_1_2 = new StkObject(L"ÉuÉãÅ[ÉoÅ[Éh");
		Elem1_1_2->AppendAttribute(new StkObject(L"ÉÅÅ[ÉJÅ[", L"ì˙éY"));
		Elem1_1_2->AppendAttribute(new StkObject(L"êF", L"îí"));
		Elem1_1->AppendChildElement(Elem1_1_2);
		StkObject* Elem1_1_1 = new StkObject(L"ÉJÉçÅ[Éâ");
		Elem1_1_1->AppendAttribute(new StkObject(L"ÉÅÅ[ÉJÅ[", L"ÉgÉàÉ^"));
		Elem1_1_1->AppendAttribute(new StkObject(L"êF", L"ê‘"));
	...
	StkObject* Elem1_4 = new StkObject(L"èÿåî");
	StkObject* Elem1_4_1 = new StkObject(L"äî");
	Elem1_4_1->AppendChildElement(new StkObject(L"Aé–", 2000000));
	Elem1_4_1->AppendChildElement(new StkObject(L"Bé–", 500000));
	Elem1_4->AppendChildElement(Elem1_4_1);
	StkObject* Elem1_4_2 = new StkObject(L"çëç¬");
	if (Mode == 1) {
		Elem1_4_2->AppendChildElement(new StkObject(L"ç¬å†", 1000000));
		Elem1_4_2->AppendChildElement(new StkObject(L"ç¬å†", 2000000));
		Elem1_4_2->AppendChildElement(new StkObject(L"ç¬å†", 1500000));
	} else {
		Elem1_4_2->AppendChildElement(new StkObject(L"ç¬å†", 2000000));
		Elem1_4_2->AppendChildElement(new StkObject(L"ç¬å†", 1500000));
		if (Mode == 2) {
			Elem1_4_2->AppendChildElement(new StkObject(L"ç¬å†", 1000001));
		} else {
			Elem1_4_2->AppendChildElement(new StkObject(L"ç¬å†", 1000000));
		}
	}
	Elem1_4->AppendChildElement(Elem1_4_2);
	StkObject* Elem1_4_3 = new StkObject(L"ã‡åî");
...
StkObject* MakeTestData4(wchar_t Name[64], int Width, int Height, int CurrentLevel = 0)
{
	StkObject* Obj = new StkObject(Name);
	if (CurrentLevel > Height) {
		Obj->AppendChildElement(new StkObject(L"Aaa", L"ìåêºìÏñk"));
		Obj->AppendChildElement(new StkObject(L"Bbb", L"ètâƒèHì~"));
		Obj->AppendChildElement(new StkObject(L"Ccc", L"È≥ñ£È±È≤"));
		Obj->AppendChildElement(new StkObject(L"Ddd", L"èƒì˜íËêH"));
	} else {
		for (int Loop = 0; Loop < Width; Loop++) {
			wchar_t Buf[25];
...
void GeneralTestCase2(StkObject* Elem1, wchar_t* Name)
{
	...
	if (lstrcmp(Elem1->GetName(), L"å¬êlÇÃç‡éY") != 0 ||
		lstrcmp(Elem1->GetFirstChildElement()->GetName(), L"èÊópé‘") != 0 ||
		lstrcmp(Elem1->GetFirstChildElement()->GetNext()->GetName(), L"éûåv") != 0 ||
		lstrcmp(Elem1->GetFirstChildElement()->GetNext()->GetNext()->GetName(), L"åªã‡") != 0 ||
		lstrcmp(Elem1->GetFirstChildElement()->GetNext()->GetNext()->GetNext()->GetName(), L"èÿåî") != 0) {
		printf("NG\r\n");
		exit(0);
	}
	printf("OK\r\n");
	...
	if (lstrcmp(Elem1->GetFirstChildElement()->GetFirstChildElement()->GetFirstAttribute()->GetStringValue(), L"ê‘") != 0 ||
		lstrcmp(Elem1->GetFirstChildElement()->GetFirstChildElement()->GetFirstAttribute()->GetNext()->GetStringValue(), L"ÉgÉàÉ^") != 0 ||
		lstrcmp(Elem1->GetFirstChildElement()->GetFirstChildElement()->GetNext()->GetFirstAttribute()->GetStringValue(), L"îí") != 0 ||
		lstrcmp(Elem1->GetFirstChildElement()->GetFirstChildElement()->GetNext()->GetFirstAttribute()->GetNext()->GetStringValue(), L"ì˙éY") != 0) {
		printf("NG\r\n");
		exit(0);
	}
	printf("OK\r\n");
...
void GeneralTestCase4()
{
	...
	}
	{
		...
		Abort(Ret != NULL && lstrcmp(Ret->GetName(), L"èÿåî") == 0 && Ret->GetFirstChildElement()->GetFirstChildElement()->GetIntValue() == 2000000);
		delete Origin;
		delete Elem1_4;
	}
	{
	...
	}
	{
		...
		int ErrorCode;
		StkObject* Origin = StkObject::CreateObjectFromXml(L"<Aaa><Bbb><Ccc Name=\"Spring\"><Ddd>èt</Ddd></Ccc><Ccc Name=\"Summer\"><Ddd>âƒ</Ddd></Ccc></Bbb><Bbb><Ccc Name=\"Fall\"><Ddd>èH</Ddd></Ccc><Ccc Name=\"Winter\"><Ddd>ì~</Ddd></Ccc></Bbb></Aaa>", &ErrorCode);
		StkObject* Target1 = StkObject::CreateObjectFromXml(L"<Aaa><Bbb><Ccc Name=\"Summer\"><Ddd>âƒ</Ddd></Ccc></Bbb></Aaa>", &ErrorCode);
		...
		StkObject* Target4 = StkObject::CreateObjectFromXml(L"<Bbb><Ccc Name=\"Fall\"><Ddd>èH</Ddd></Ccc></Bbb>", &ErrorCode);
		StkObject* Target5 = StkObject::CreateObjectFromXml(L"<Ccc Name=\"Spring\"><Ddd>èt</Ddd></Ccc>", &ErrorCode);
		StkObject* Target6 = StkObject::CreateObjectFromXml(L"<Ddd>ì~</Ddd>", &ErrorCode);
		StkObject* Target7 = StkObject::CreateObjectFromXml(L"<Aaa><Bbb><Ccc Name=\"Spring\"><Ddd>èt</Ddd></Ccc></Bbb><Bbb><Ccc Name=\"Winter\"><Ddd>ì~</Ddd></Ccc></Bbb></Aaa>", &ErrorCode);
	...
	}
	{
		...
		int ErrorCode;
		StkObject* Origin = StkObject::CreateObjectFromXml(L"<Aaa><Bbb><Ccc Name=\"Spring\"><Ddd>èt</Ddd></Ccc><Ccc Name=\"Summer\"><Ddd>âƒ</Ddd></Ccc></Bbb><Bbb><Ccc Name=\"Fall\"><Ddd>èH</Ddd></Ccc><Ccc Name=\"Winter\"><Ddd>ì~</Ddd></Ccc></Bbb></Aaa>", &ErrorCode);
		StkObject* Target1 = StkObject::CreateObjectFromXml(L"<Aaa><Bbb><Ccc Name=\"Summer\"><Ddd>ì~</Ddd></Ccc></Bbb></Aaa>", &ErrorCode);
		StkObject* Target2 = StkObject::CreateObjectFromXml(L"<Aaa><Bbb><Ddd>âƒ</Ddd></Bbb></Aaa>", &ErrorCode);
		StkObject* Target3 = StkObject::CreateObjectFromXml(L"<Aaa><Ccc Name=\"Winter\"><Ddd>ì~</Ddd></Ccc></Aaa>", &ErrorCode);
		StkObject* Target4 = StkObject::CreateObjectFromXml(L"<Aaa><Ddd>âƒ</Ddd></Aaa>", &ErrorCode);
		StkObject* Target5 = StkObject::CreateObjectFromXml(L"<Bbb><Ddd>âƒ</Ddd></Bbb>", &ErrorCode);
		StkObject* Target6 = StkObject::CreateObjectFromXml(L"<Aaa>ì~</Aaa>", &ErrorCode);
		StkObject* Target7 = StkObject::CreateObjectFromXml(L"<Aaa><Bbb><Ccc Name=\"Spring\"><Ddd>èt</Ddd></Ccc><Ccc Name=\"Winter\"><Ddd>ì~</Ddd></Ccc></Bbb></Aaa>", &ErrorCode);
	...
	}
	{
		...
		int ErrorCode;
		StkObject* Origin = StkObject::CreateObjectFromXml(L"<Aaa><Bbb Type=\"ï∂éö\"><Ccc Name=\"ãGêﬂ\"><Ddd>èt</Ddd><Ddd>âƒ</Ddd><Ddd>èH</Ddd><Ddd>ì~</Ddd></Ccc><Ccc Name=\"ï˚äp\"><Ddd>ìå</Ddd><Ddd>êº</Ddd><Ddd>ìÏ</Ddd><Ddd>ñk</Ddd></Ccc></Bbb><Bbb Type=\"éÅñº\"><Ccc Name=\"â∆ë∞\"><Ddd>í|ì‡êLñÁ</Ddd><Ddd>èÌå©éÈî¸</Ddd><Ddd>ñÿë∫óDéq</Ddd></Ccc><Ccc Name=\"âÔé–ÉÅÉìÉoÅ[\"><Ddd>éRìcëæòY</Ddd><Ddd>óÈñÿàÍòY</Ddd></Ccc></Bbb></Aaa>", &ErrorCode);
		StkObject* Target1 = StkObject::CreateObjectFromXml(L"<Bbb Type=\"ï∂éö\"/>", &ErrorCode);
		StkObject* Target2 = StkObject::CreateObjectFromXml(L"<Ccc Name=\"ï˚äp\"/>", &ErrorCode);
		StkObject* Target3 = StkObject::CreateObjectFromXml(L"<Ccc Name=\"ãGêﬂ\"/>", &ErrorCode);
		StkObject* Target4 = StkObject::CreateObjectFromXml(L"<Ddd>ìÏ</Ddd>", &ErrorCode);
		StkObject* Target5 = StkObject::CreateObjectFromXml(L"<Ddd>ì~</Ddd>", &ErrorCode);
		...
		Abort(lstrcmp(Ret124->GetStringValue(), L"ìÏ") == 0 && Ret125 == NULL &&
			  Ret134 == NULL && lstrcmp(Ret135->GetStringValue(), L"ì~") == 0);
		delete Origin;
		delete Target1;
		delete Target2;
		delete Target3;
...
void JsonEncodingTest2()
{
	...
	if (lstrcmp(JsonTxt2, L"\"å¬êlÇÃç‡éY\" : {\r\n  \"èÊópé‘\" : {\r\n    \"") != 0) {
		printf("NG\r\n");
		exit(0);
	}
	lstrcpy(JsonTxt2, L"");
...
void PerformanceTest()
{
	...
	for (int Loop = 2; Loop < 10; Loop++) {
		lstrcpy(JsonTxt, L"");
		StkObject* ObjWid = MakeTestData3(L"Ç†Ç¢Ç§Ç¶Ç®Ç©Ç´Ç≠ÇØÇ±Ç≥ÇµÇ∑ÇπÇªÇΩÇøÇ¬ÇƒÇ∆Ç»Ç…Ç ÇÀÇÃÇÕÇ–Ç”Ç÷ÇŸ", 2, Loop);
